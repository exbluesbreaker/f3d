...
void vtkF3DImguiActor::RenderFileName()
{
  if (!this->FileName.empty())
  {
    ...
    if ((this->HDRIFileNameVisible) && (!this->HDRIFileName.empty()))
    {
      ...
      hdriWinSize.y += 2.f * ImGui::GetStyle().WindowPadding.y;
      totalWidth += hdriWinSize.x + 2.f * ImGui::GetStyle().WindowPadding.x;
    }
...
void vtkF3DImguiActor::RenderHDRIFileName()
{
  if (!this->HDRIFileName.empty())
  {
    ...
    if ((this->FileNameVisible) && (!this->FileName.empty()))
    {
      ...
      fileWinSize.y += 2.f * ImGui::GetStyle().WindowPadding.y;
      totalWidth += fileWinSize.x + 2.f * ImGui::GetStyle().WindowPadding.x;
      winOffsetX = fileWinSize.x + 2.f * ImGui::GetStyle().WindowPadding.x;
    }
